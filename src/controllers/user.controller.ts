...
const createUser = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: `El usuario ${newUser.username} a sido creado` });
  } catch (error) {
    return res
      .status(500)
...
const updateUser = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const updateById = await userModel.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    return updateById
...
const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    return deleteById
      ? res.status(200).json({ message: `Se ha eliminado al usuario` })
      : res
          .status(404)
